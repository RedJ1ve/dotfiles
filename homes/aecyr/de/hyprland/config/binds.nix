...
in {
  wayland.windowManager.hyprland.settings = {
    ...
    bind = [
      ...
      "$MODSHIFT, L, exec, ${locker}" # lock the screen with hyprlock
      "$MODSHIFT,E,exit," # exit Hyprland session

      # Daily Applications
      "$MOD,F1,exec,schizofox" # browser
      ''$MOD,F2,exec,run-as-service "${defaults.fileManager}"'' # file manager
      ''$MOD,Q,exec,run-as-service $(foot${lib.optionalString config.programs.foot.server.enable "client"})'' # terminal
      ''$MODSHIFT,Q,exec,[floating] run-as-service $(foot${lib.optionalString config.programs.foot.server.enable "client"})'' # floating terminal (TODO)
      ''$MOD,R,exec, killall anyrun || run-as-service $(anyrun)'' # alternative application launcher with more features
